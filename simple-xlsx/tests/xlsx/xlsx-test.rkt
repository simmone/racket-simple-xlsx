   ...
   (test-case
    ...
    (let ([xlsx (new xlsx%)])
      (send xlsx add-data-sheet 
            #:sheet_name "测试1" 
            #:sheet_data (list
                          (list 1 (seconds->date (find-seconds 0 0 0 17 9 2018) #f))
                          (list 2 (seconds->date (find-seconds 0 0 0 18 9 2018) #f))
                          (list 3 (seconds->date (find-seconds 0 0 0 19 9 2018) #f))
                          ))
      
      (let* ([sheets (get-field sheets xlsx)]
             [data_sheet (data-sheet-rows (sheet-content (first sheets)))])
        (check-equal? (second (list-ref data_sheet 0)) 43359)
        (check-equal? (second (list-ref data_sheet 1)) 43360)
        (check-equal? (second (list-ref data_sheet 2)) 43361))))
