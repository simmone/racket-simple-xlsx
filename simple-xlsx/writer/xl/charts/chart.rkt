...
(lambda ()
(let loop ([loop_list (send xlsx get-range-data (data-range-sheet_name x_data_range) (data-range-range_str x_data_range))]
           [idx 0])
  (when (not (null? loop_list))
        (prinf "<c:pt idx=\"~a\"><c:v>~a</c:v></c:pt>" idx (car loop_list))
        (loop (cdr loop_list) (add1 idx))))))|</c:numCache></c:numRef></c:cat>@|(with-output-to-string
(lambda ()
  (let loop ([loop_list y_data_range_list]
    ...
    (when (not (null? loop_list))
          (let* ([y_data_serial (car loop_list)]
                 ...
                 [data_range (data-serial-data_range y_data_range)]
            ...
            (printf "<c:f>~a!~a</c:f>" sheet_name range_str)
